...
- 詳細レポートは視覚的に区別されたセクションとして表示。注意: Gmail等の多くのメールクライアントはCSSベースの折りたたみ機能（`<details>`タグやcheckboxハック）をサポートしていないため、コンテンツは常に表示される。
...
- **mail_utils.py**：メール送信・SMTP 設定・分類別メール本文生成・レポートセクション生成のユーティリティ。メール配信機能を提供し、保有状況に応じて分類されたメール本文を生成する。メールクライアントの制限により、折りたたみ機能は実装していない。
