...
- **見出し階層の最適化**：視認性向上のため、見出し階層を最適化：
  - 個別レポートファイル：メール件名との重複を回避
  - メール本文：カテゴリー名（「保有銘柄」等）はメール件名に含まれるため本文には表示せず、銘柄名を主要な見出しとして使用
  - 銘柄間の区切りを明確化し、視覚的に識別しやすいスタイリングを適用
- **折りたたみ機能**：メール本文では、詳細レポートを折りたたみ可能：
  - デフォルトでは折りたたまれた状態で表示され、クリック/タップで展開
  - 長いレポートでも一覧性を保ち、必要な銘柄の詳細のみ確認可能
- **レポート簡略化機能**：AI分析の売買判断が「ホールド」の場合、レポートを簡略化可能：
  - 環境変数 `SIMPLIFY_HOLD_REPORTS` で制御（デフォルト: `true`）
  - 簡略化時は、銘柄名、現在価格、ホールド理由の要約のみ表示
  - 詳細なレポートが必要な場合は `SIMPLIFY_HOLD_REPORTS=false` に設定
  - メール本文の長さを削減し、読みやすさを向上
