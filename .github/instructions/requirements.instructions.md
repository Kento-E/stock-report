...
- 詳細レポートは折りたたみ可能で、デフォルトでは折りたたまれた状態で表示。Gmail互換のCSS checkboxハック方式（`<input type="checkbox">` + `<label>` + CSSの`:checked`疑似クラス）を使用し、Gmailでも正しく動作する。
...
- **mail_utils.py**：メール送信・SMTP 設定・分類別メール本文生成・Gmail互換折りたたみセクション生成のユーティリティ。メール配信機能を提供し、保有状況に応じて分類されたメール本文を生成する。checkboxハック方式を使用したGmail対応の折りたたみ機能を実装。
