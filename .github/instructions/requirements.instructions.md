...
- **stock_loader.py**：YAML銘柄リストの読み込み、通貨判定、銘柄分類機能、口座種別による課税計算（190行）
- **data_fetcher.py**：Yahoo Finance APIとdefeatbeta-apiによるデータ取得（93行）
- **ai_analyzer.py**：Claude API/Gemini APIによる分析処理、保有状況プロンプト生成、口座種別を考慮した税引後損益計算（212行）
...
- 2025-10-16: 口座種別による課税計算機能を実装。stocks.yamlにaccount_typeフィールドを追加（特定/NISA/旧NISA）。特定口座では譲渡益に20.315%課税、NISA・旧NISAは非課税。AI分析レポートに税引後損益を表示し、投資判断の材料として活用可能に。
