...
- **PR初回作成時（最初の `report_progress` 呼び出し）から、必ず下記の `<!-- for GitHub Copilot review rule -->` HTMLコメントブロックを含めること**
  - 後から追加すると、既に実施済みのレビューには反映されないため、**初回から必須**
...
- PR・Issue へのコメントでコミットハッシュを参照する際は、**バッククォートで囲まず**、前後を半角スペースで区切った素のハッシュを記載してください。
  - ✅ 正しい例: `コミット a1b2c3d で対応しました。`
  - ❌ 誤った例: `コミット \`a1b2c3d\` で対応しました。`
- バッククォートで囲むと GitHub がリンクとして認識しないため、上記のルールを必ず守ってください。
