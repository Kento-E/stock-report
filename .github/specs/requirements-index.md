...
銘柄株の動向やトレンドを Claude Sonnet（AI）で週2回（火曜日・木曜日）調査・分析し、HTML レポートとして自動生成・メール配信するシステム。
...
5. **自動実行**: GitHub Actions による週2回（火曜日・木曜日）自動実行
...
- 実行時刻: JST 9:00（火曜日・木曜日）
- 実行頻度を週2回に制限することで、Gemini API RPD（1日あたりのリクエスト数）を約60%削減
