...
リポジトリを外部からの不正な改修から保護するため、以下のセキュリティ機能を実装・推奨しています：

- **CODEOWNERS設定**: `.github/CODEOWNERS` ファイルによる自動レビュアー割り当て
  - すべてのファイルに対してリポジトリオーナーとCopilotをコードオーナーとして設定
  - プルリクエスト作成時に自動的にレビュアーが割り当てられる
- **ブランチ保護ルール**: GitHub Settings でのブランチ保護設定を推奨
  - mainブランチへの直接プッシュを禁止
  - プルリクエストとレビュー承認を必須化
  - ステータスチェック（テスト）の成功を必須化
  - コードオーナーのレビューを必須化
- **2要素認証（2FA）**: アカウント乗っ取り防止のため、オーナーアカウントで2FAを有効化することを推奨
- **コミット署名**: GPG署名によるコミット作成者の正当性保証を推奨

**詳細な設定方法は <a>.github/docs/SECURITY_SETUP.md</a> を参照してください。**
