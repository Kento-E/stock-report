...
def load_investment_preferences(filepath="data/investment_preferences.yaml"):
    ...
    if not os.path.exists(filepath):
        print(
            f"警告: 投資志向性設定ファイル '{filepath}' が見つかりません。デフォルト設定を使用します。"
        )
...
def _validate_preferences(prefs):
    ...
    if (
        "investment_horizon" in prefs
        and prefs["investment_horizon"] not in VALID_INVESTMENT_HORIZONS
    ):
        raise ValueError(
...
def generate_preference_prompt(prefs=None):
    ...
    if prefs is None:
        try:
            ...
            print(
                f"警告: 投資志向性設定の読み込みに失敗しました。デフォルト設定を使用します。エラー: {e}"
            )
    ...
    prompt_parts.append(
        f"- 投資スタイル: {INVESTMENT_STYLE_LABELS.get(investment_style, investment_style)}"
    )

    # リスク許容度
    risk_tolerance = prefs.get("risk_tolerance", "medium")
    prompt_parts.append(
        f"- リスク許容度: {RISK_TOLERANCE_LABELS.get(risk_tolerance, risk_tolerance)}"
    )

    # 投資期間
    investment_horizon = prefs.get("investment_horizon", "medium")
    prompt_parts.append(
        f"- 投資期間: {INVESTMENT_HORIZON_LABELS.get(investment_horizon, investment_horizon)}"
    )

    # 売買頻度
    trading_frequency = prefs.get("trading_frequency", "medium")
    prompt_parts.append(
        f"- 売買頻度: {TRADING_FREQUENCY_LABELS.get(trading_frequency, trading_frequency)}"
    )
