...
- 保有中の銘柄については、売却だけでなく買い増しの検討も含めて判断されます
...
Daily Stock Reportワークフローは、任意のブランチから手動で実行できます。PRブランチでも実行可能なため、マージ前に実際のメール送信をテストできます。

### 手動実行の手順

1. GitHubリポジトリの **「Actions」** タブを開く
2. 左サイドバーから **「Daily Stock Report」** ワークフローを選択
3. 右上の **「Run workflow」** ボタンをクリック
4. **「Use workflow from」** ドロップダウンから実行したいブランチを選択
   - `main`: 本番環境の設定でテスト
   - PRブランチ: マージ前に変更内容をテスト
...
- 実行ログで各ステップの詳細を確認可能
- 設定したメールアドレスに実際にレポートが送信されます

### 注意事項

- Secretsの設定が完了していることを確認してください（設定方法は上記参照）
- PRブランチで実行する場合、そのブランチに `.github/workflows/report.yml` ファイルが存在する必要があります
- メール送信が実際に行われるため、テストアドレスを使用することを推奨します
