...
Gemini APIの無料枠は20リクエスト/日です。銘柄数が多い場合、日次処理上限を設定することで、無料枠の範囲内で分析を実行できます。

- **デフォルト値**: `18`
- **設定可能範囲**: `1`～`20`
- **動作**: 銘柄数が上限を超える場合、日付ベースのローテーションで分散処理
  - 各実行日に異なる銘柄セットを自動選択
- **Claude APIを使用する場合**: この制限は適用されず、全銘柄を一度に処理
...
- **Copilot Agents**: GitHub Copilot エージェント設定（タスク別の指示）は `.github/agents/` ディレクトリにまとめられています。詳細は `.github/agents/README.md` を参照してください。
...
- 詳細な動作確認手順は `docs/TEST.md` を参照してください。
