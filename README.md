...
- 🚀 **IPOレポート**: 上場予定銘柄（7日以内）を別途レポート・配信
...
├── ipo_loader.py          # YAML上場予定銘柄リスト読み込み
...
本プロジェクトでは、上場予定の銘柄リストを `data/ipo_stocks.yaml` ファイル（YAML形式）で管理しています。

### 上場予定銘柄リストの編集方法

1. GitHub上で `data/ipo_stocks.yaml` ファイルを開く
2. 編集ボタン（鉛筆アイコン）をクリック
3. 上場予定銘柄を追加・削除・編集
4. 変更をコミット
...
- `currency` (任意): 通貨単位（円、ドル、ユーロ等。未設定の場合は銘柄シンボルから自動判定）
- `note` (任意): メモ・注記

**IPOレポートの動作:**

- 処理実行日から **7日以内** に上場予定の銘柄を自動的に抽出してレポート生成
- 上場予定日が設定されていない銘柄も含まれます
- レポートは既存の通常レポートとは別に生成され、専用のメールで配信されます
- メール件名は「上場予定銘柄レポート」となります
- ファイルが存在しない、または銘柄が登録されていない場合は、IPOレポート処理をスキップします（エラーにはなりません）
...
- **ユニットテスト**: 各モジュールの個別機能をテスト
  ...
  - `test_ipo_loader.py`: 上場予定銘柄リスト読み込み、日付フィルタリング
  - `test_data_fetcher.py`: データ構造の検証
  - `test_report_generator.py`: HTMLレポート生成、IPOレポート生成
